 ...
 */
class DateModelElement extends CharModelElement
{
	protected $display_selects = false;
	
	protected $now_on_create = true;
	
	protected $selects_values = ['dd' => '', 'mm' => '', 'yyyy' => ''];
	...
	public function validate()
	{
		...
		if(get_class($this) == 'DateModelElement')
		{
			...
			{
				$parts = explode('-', I18n::dateForSQL($this -> value));

				if(false === checkdate(intval($parts[1]), intval($parts[2]), intval($parts[0])))
					$this -> error = $this -> chooseError('format', '{error-undefined-value}');

				if(!$this -> error && null !== $years = $this -> getYearsRange())
					if(intval($parts[0]) < $years['from'] || intval($parts[0]) > $years['to'])
						$this -> error = $this -> chooseError('format', '{error-undefined-value}');
			}
		}
	...
	public function displayAdminFilter(mixed $data)
	{
		...
		$html .= "name=\"".$this -> name."-from\" value=\"".$value_from."\" /></td></tr><tr><td>\n";
		$html .= I18n::locale('date-to')."</td><td><input type=\"text\"class=\"".$css_class."\" ";
		$html .= "name=\"".$this -> name."-to\" value=\"".$value_to."\" />\n";
		$html .= "</td></tr></table>\n";
	...
	public function getYearsRange(bool $forced = false)
	{
		if(!is_string($this -> years) && !is_array($this -> years))
			return $forced ? ['from' => 1920, 'to' => intval(date('Y'))] : null;
		
		$years = is_string($this -> years) ? explode(',', $this -> years) : $this -> years;

		if(isset($years[0], $years[1]) && is_numeric($years[0]) && is_numeric($years[1]) && intval($years[0]) < intval($years[1]))
			return [
				'from' => abs(intval($years[0])),
				'to' => abs(intval($years[1]))
			];
	...
	public function setValue($value)
	{
		...
		{
			$this -> value = '';
			$this -> selects_values = ['dd' => '', 'mm' => '', 'yyyy' => ''];
	...
	public function getValue()
	{
		...
		if(isset($arguments[0]) && $arguments[0] == 'sql' && !is_null($this -> value))
		{
	...
	public function displayAsSelects()
	{
		...
		$years = $this -> getYearsRange(true);
		
		for($i = $years['to']; $i >= $years['from']; $i --)
		{
			$selected = ($value && $value == $i) ? " selected=\"selected\"" : "";
			$html .= "<option value=\"".$i."\"".$selected.">".$i."</option>\n";
		}
